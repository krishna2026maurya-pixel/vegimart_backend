...
import VendorProduct from '@/lib/models/VendorProduct';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [data, total] = await Promise.all([
      VendorProduct.find(query).sort({ createdAt: -1 }).skip((page-1)*limit).limit(limit).lean(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await VendorProduct.create(body);
