...
export default function EditCategoryTypePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/category-types/${id}`);
        ...
        if (json.data) {
          ...
          if (json.data.image) {
            ...
            setImagePreview(json.data.image.startsWith('/') || json.data.image.startsWith('http') ? json.data.image : `/uploads/${json.data.image}`);
          }
        }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Category Type</h1></div>
      </div>
