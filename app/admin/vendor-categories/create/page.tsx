...
export default function CreateVendorCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error);
      router.push('/admin/vendor-categories');
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSaving(false);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">New Vendor Category</h1>
          <p className="text-sm text-gray-500">नयी vendor category जोड़ें</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input name="category_name" value={form.category_name} onChange={handleChange} required
